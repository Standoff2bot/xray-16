-- details_lod_gpu.s - GPU instanced grass rendering variant
-- Uses lod_gpu vertex shader with compute-culled instance buffers
...
--	pre_pass --
	if ( opt:dx10_msaa_alphatest_atoc() ) then
		shader	:begin	("lod_gpu","lod_atoc")
		details_lod_gpu.pass_setup_common(shader, t_base, t_second, t_detail)
		shader  :dx10color_write_enable( false, false, false, false)
		shader	:dx10atoc( true )
	end

--	main pass --
	shader	:begin	("lod_gpu","lod")
	details_lod_gpu.pass_setup_common(shader, t_base, t_second, t_detail)
	if ( opt:dx10_msaa_alphatest_atoc() ) then
