        ...
        auto ProcessTileRange = [](TileBakeContext& ctx, const TaskRange<size_t>& range)
            {
                ...
                for (size_t idx = range.begin(); idx != range.end(); ++idx)
                {
                    ...
                    xr_vector<float> slot_min(slot_cell_count, FLT_MAX);
                    xr_vector<float> slot_max(slot_cell_count, -FLT_MAX);
                    xr_vector<float> slot_fallback_min(slot_cell_count, FLT_MAX);
                    xr_vector<float> slot_fallback_max(slot_cell_count, -FLT_MAX);

                    float tile_min_y = FLT_MAX;
                    float tile_max_y = -FLT_MAX;
                    float tile_world_min_x = FLT_MAX;
                    float tile_world_min_z = FLT_MAX;
                    float tile_world_max_x = -FLT_MAX;
                    float tile_world_max_z = -FLT_MAX;
                    ...
                    for (size_t local = 0; local < count; ++local)
                    {
                        ...
                        const float slot_world_max_x = slot_world_min_x + DETAIL_SLOT_SIZE;
                        const float slot_world_max_z = slot_world_min_z + DETAIL_SLOT_SIZE;

                        tile_world_min_x = std::min(tile_world_min_x, slot_world_min_x);
                        tile_world_min_z = std::min(tile_world_min_z, slot_world_min_z);
                        tile_world_max_x = std::max(tile_world_max_x, slot_world_max_x);
                        tile_world_max_z = std::max(tile_world_max_z, slot_world_max_z);
                        ...
                        slot_fallback_min[lut_index] = std::min(slot_fallback_min[lut_index], slot_base);
                        slot_fallback_max[lut_index] = std::max(slot_fallback_max[lut_index], slot_top);
                    ...
                    float query_min_y = tile_min_y;
                    float query_max_y = tile_max_y;

                    if (query_min_y == FLT_MAX)
                    {
                        query_min_y = -10.f;
                        query_max_y = 10.f;
                    }
                    ...
                    tile_min_y = query_min_y;
                    tile_max_y = query_max_y;

                    if (tile_world_min_x == FLT_MAX || tile_world_max_x == -FLT_MAX || tile_world_min_z == FLT_MAX || tile_world_max_z == -FLT_MAX)
                    {
                        const float default_min_x = ctx.header->slot_min_x(key.first * ctx.tile_span);
                        const float default_min_z = ctx.header->slot_min_z(key.second * ctx.tile_span);
                        tile_world_min_x = default_min_x;
                        tile_world_min_z = default_min_z;
                        tile_world_max_x = default_min_x + ctx.tile_span * DETAIL_SLOT_SIZE;
                        tile_world_max_z = default_min_z + ctx.tile_span * DETAIL_SLOT_SIZE;
                    }
                    ...
                    static thread_local CDB::COLLIDER tile_xrc;
                    const bool has_level = (g_pGameLevel != nullptr);
                    const CDB::RESULT* tile_results = nullptr;
                    CDB::TRI* tris = nullptr;
                    Fvector* verts = nullptr;
                    size_t tile_tri_count = 0;

                    if (has_level)
                    {
                        Fvector bC, bD;
                        bC.set(
                            (tile_world_min_x + tile_world_max_x) * 0.5f,
                            (query_min_y + query_max_y) * 0.5f,
                            (tile_world_min_z + tile_world_max_z) * 0.5f);

                        const float horizontal_pad = DETAIL_SLOT_SIZE;
                        const float vertical_pad = 5.f;
                        bD.set(
                            std::max((tile_world_max_x - tile_world_min_x) * 0.5f + horizontal_pad, DETAIL_SLOT_SIZE * 0.5f),
                            ((query_max_y - query_min_y) * 0.5f) + vertical_pad,
                            std::max((tile_world_max_z - tile_world_min_z) * 0.5f + horizontal_pad, DETAIL_SLOT_SIZE * 0.5f));

                        tile_xrc.box_query(CDB::OPT_FULL_TEST, g_pGameLevel->ObjectSpace.GetStaticModel(), bC, bD);
                        tile_tri_count = tile_xrc.r_count();
                        if (tile_tri_count > 0)
                        {
                            tile_results = tile_xrc.r_begin();
                            tris = g_pGameLevel->ObjectSpace.GetStaticTris();
                            verts = g_pGameLevel->ObjectSpace.GetStaticVerts();
                        }
                    }

                    const float ray_origin_y = query_max_y + 2.f;
                    Fvector ray_dir;
                    ray_dir.set(0.f, -1.f, 0.f);

                    u32 tile_debug_logged_hits = 0;
                    u32 tile_debug_total_hits = 0;
                    ...
                    for (u32 z = 0; z < ctx.tile_resolution; ++z)
                    {
                        for (u32 x = 0; x < ctx.tile_resolution; ++x)
                        {
                            ...
                            const float sample_world_x = tile_world_min_x + (tile_world_max_x - tile_world_min_x) * norm_x;
                            const float sample_world_z = tile_world_min_z + (tile_world_max_z - tile_world_min_z) * norm_z;

                            Fvector origin;
                            origin.set(sample_world_x, ray_origin_y, sample_world_z);

                            float best_height = -FLT_MAX;
                            if (tile_tri_count > 0)
                            {
                                float r_u, r_v, r_range;
                                for (size_t tri_idx = 0; tri_idx < tile_tri_count; ++tri_idx)
                                {
                                    const CDB::TRI& tri = tris[tile_results[tri_idx].id];
                            ...
                            float final_height;
                            if (best_height == -FLT_MAX)
                            {
                                const float fallback_height = slot_fallback_min[lut_index] != FLT_MAX ? slot_fallback_min[lut_index] : query_min_y;
                            ...
                            else
                            {
                                ...
                                ++valid_samples;
                                ++tile_debug_total_hits;
                                if (tile_debug_logged_hits < 8)
                                {
                                    Msg("~ [GPUGrass] Tile (%d,%d) sample (%u,%u) world(%.2f, %.2f) height %.3f",
                                        key.first,
                                        key.second,
                                        x,
                                        z,
                            ...
                            if (slot_min[lut_index] == FLT_MAX || final_height < slot_min[lut_index])
                                slot_min[lut_index] = final_height;
                            if (slot_max[lut_index] == -FLT_MAX || final_height > slot_max[lut_index])
                                slot_max[lut_index] = final_height;
                    ...
                    if (local_min_height == FLT_MAX)
                        local_min_height = query_min_y;
                    if (local_max_height == -FLT_MAX)
                        local_max_height = query_max_y;

                    if (valid_samples == 0 && ctx.tile_sample_count > 0)
                        valid_samples = ctx.tile_sample_count;

                    tile_valid_samples[idx] = valid_samples;
                    if (tile_debug_total_hits > 0)
                    {
                        Msg("~ [GPUGrass] Tile (%d,%d) valid height hits: %u / %u samples",
                            key.first,
                            key.second,
                            tile_debug_total_hits,
                            ctx.tile_sample_count);
                    }
                    ...
                    for (size_t cell = 0; cell < slot_cell_count; ++cell)
                    {
                        if (slot_min[cell] == FLT_MAX)
                            slot_min[cell] = (slot_fallback_min[cell] != FLT_MAX) ? slot_fallback_min[cell] : query_min_y;
                        if (slot_max[cell] == -FLT_MAX)
                            slot_max[cell] = (slot_fallback_max[cell] != -FLT_MAX) ? slot_fallback_max[cell] : query_max_y;
                        else
                            slot_max[cell] = std::max(slot_max[cell], slot_fallback_max[cell]);
                    }
