...
void CBlender_accum_direct_cascade::Compile(CBlender_Compile& C)
{
    ...
    case SE_SUN_MIDDLE:
        C.r_Pass("accum_volume", "accum_sun_cascade", false, TRUE, FALSE, blend, nvrhi::BlendFactor::One, dest);
    ...
    case SE_SUN_FAR: // far pass, only stencil clipping performed
        C.r_Pass("accum_volume", "accum_sun_cascade_far", false, TRUE, FALSE, blend, nvrhi::BlendFactor::One, dest);
        C.r_Sampler_rtf("s_position", r2_RT_P);
        C.r_Sampler_rtf("s_normal", r2_RT_N);
        C.r_Sampler_clw("s_material", r2_material);
        C.r_Sampler_rtf("s_accumulator", r2_RT_accum);
