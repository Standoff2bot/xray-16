...
#   error No graphics API selected or enabled!
#endif
    RCache.Render(nvrhi::PrimitiveType::TriangleList, v_offset, 0, 12, i_offset, 20);
...
void dxEnvironmentRender::RenderClouds(CEnvironment& env)
{
    ...
    RCache.Render(nvrhi::PrimitiveType::TriangleList, v_offset, 0, env.CloudsVerts.size(), i_offset, env.CloudsIndices.size() / 3);
